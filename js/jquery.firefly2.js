/*
 * fireflyTwo v1.1.1 - jQuery plugin
 *
 * An updated version of Dharmveer Motyar's fireflyTwo plugin.
 * Creates an effect of fireflyTwos flying around your page.
 *
 * @example  $.fireflyTwo()
 *
 *           // to stop and start
...
(function($){
    ...
    $.fireflyTwo.start = function(){
        start = parseInt((new Date).getTime()/1000)
        $(window).bind('resize.fireflyTwo', function(){
    ...
    $.fireflyTwo.delay = function(callback, milliseconds){
    ...
    $.fireflyTwo.stop = function(){
        $(window).unbind('resize.fireflyTwo')
    ...
    $.fireflyTwo.create = function(i){
        var spark = $('<img>').attr({'src':i}).hide()
        $("#firefly2").append(spark)
    ...
    $.fireflyTwo.move = function(image){
    ...
    $.fireflyTwo.check_time = function(){
        if (parseInt((new Date).getTime()/1000) > (start+ffs.limit)) {
            ff.stop()
        } else {
            window.setTimeout('$.fireflyTwo.check_time()', 1000)
        }
    }
    ...
    $.fireflyTwo.random = function(a,b){
